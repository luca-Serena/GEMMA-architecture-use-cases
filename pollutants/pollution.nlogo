...
to setup [initial-population]
  ...
  set petroil-total initial-population
  set GPL-total 0
  set electric-total 0
...
to go
  ...
  ask patches [
    if any? turtles-here with [fuel = "petroil"][
      set pollution pollution + 1.2;+ initial-population / 200;1
    ]
    if any? turtles-here with [fuel = "GPL"][
      set pollution pollution + 0.6;+ initial-population / 200;1
    ]
    set pollution pollution + 0.00008 * electric-total
    set pollution pollution - 0.02  ;at each time-step decrease pollution
    if pollution < 0 [
      set pollution 0
    ]
  ]

  diffuse pollution 0.4

  ask patches [ pollute ]
  tick
end

to update-vehicles [petroil GPL electric]
  let diff-petroil    petroil  -  petroil-total
  let diff-GPL        GPL      -  GPL-total
  let diff-electric   electric -  electric-total

  ;show (list (count turtles with [fuel = "petroil"] ) (count turtles with [fuel = "GPL"]) (count turtles with [fuel = "electric"]))
  if diff-petroil < 0 [
    ask n-of abs diff-petroil turtles with [fuel = "petroil"][set fuel  "temp"]
  ]

  if diff-GPL < 0 [
    ask n-of abs diff-GPL turtles with [fuel = "GPL"][set fuel  "temp"]
  ]

  if diff-GPL > 0 [
    ask n-of diff-GPL turtles with [fuel = "temp"] [set fuel "GPL"]
  ]

   if diff-electric > 0 [
    ask n-of diff-electric turtles with [fuel = "temp"] [set fuel "electric"]
  ]

  ask turtles with [fuel = "GPL"] [set color 105]
  ask turtles with [fuel = "electric"] [set color 65]

  set petroil-total petroil
  set GPL-total GPL
  set electric-total electric
end
...
to-report get-pollution
  report sum [ pollution ] of patches
end
...
"" ""
PENS
"petroil" 1.0 0 -16777216 true "" "plot petroil-total"
"GPL" 1.0 0 -13345367 true "" "plot GPL-total"
"electric" 1.0 0 -13840069 true "" "plot electric-total"
